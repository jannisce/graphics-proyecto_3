...
Intersect Cube::rayIntersect(const glm::vec3& rayOrigin, const glm::vec3& rayDirection) const {
    ...
    float tmin_final = glm::max(glm::max(tmin.x, tmin.y), tmin.z);
    float tmax_final = glm::min(glm::min(tmax.x, tmax.y), tmax.z);
...
glm::vec3 Cube::calculateNormal(const glm::vec3& point) const {
    ...
    float bias = 1e-4; // Small bias to handle numerical precision issues
